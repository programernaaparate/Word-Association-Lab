...
-- Associations: 110
-- Logic challenges: 152
-- Relation challenges: 157


INSERT INTO users (username, password_hash, role, points, level)
VALUES
  ('admin_seed', '$2b$10$nXOsM8TIv.LPqnRtkCwjUuRiHdcyysPRICxfCaOYowL0foMQLml7u', 'admin', 1500, 2),
  ('demo_mia', '$2b$10$yTRAQbg2pRjqc5kOzT637e0jtUCaPkQtL6Mru3BPLnTQKX4W56C72', 'user', 570, 1),
  ('demo_nikola', '$2b$10$Q6JafKMOhLvy0kp.ZWH4Pez1SN0QwWr9iTlm8Xrf9SN7OV9pbllkO', 'user', 1210, 2),
  ('demo_lana', '$2b$10$XPTM0VF8hLwB6NJLVoqn3.YS5xBzuJ/5DBfr.nFzDUUJVeNLbS34m', 'user', 880, 1),
  ('demo_marko', '$2b$10$1iTxWeCemSGO7wl215WcJuK1gypf.LwLAgNnvWS.8keyRUBZqWl2K', 'user', 1045, 2),
  ('demo_sara', '$2b$10$EIFI28wfeYQEziATzICwKu625h.jm67clHA3oQ6t6RXF4IzsOtbQm', 'user', 690, 1)
...
FROM (
  ...
  UNION ALL SELECT 'odd-one-out' AS mode, '["Pregovori","Ambasada","Sporazum","Jezgro"]' AS words_json, 'Jezgro' AS answer, 'Tri pojma vode ka "Diplomatija", a jedan ne pripada toj grupi.' AS hint, 'Istorija' AS category, 'Tesko' AS difficulty
  ...
  UNION ALL SELECT 'odd-one-out' AS mode, '["Talas","So","Plaža","Jezgro"]' AS words_json, 'Jezgro' AS answer, 'Tri pojma vode ka "More", a jedan ne pripada toj grupi.' AS hint, 'Priroda' AS category, 'Srednje' AS difficulty
  ...
  UNION ALL SELECT 'odd-one-out' AS mode, '["Mašina","Program","Senzor","Jezgro"]' AS words_json, 'Jezgro' AS answer, 'Tri pojma vode ka "Robot", a jedan ne pripada toj grupi.' AS hint, 'Tehnologija' AS category, 'Lako' AS difficulty
...
SELECT seed.left_word, seed.right_word, seed.relation, seed.category, seed.difficulty, seed.hint
FROM (
  SELECT 'Diplomatija' AS left_word, 'Pregovori' AS right_word, 'Asocijacija' AS relation, 'Istorija' AS category, 'Tesko' AS difficulty, 'Povezi pojam "Diplomatija" sa jednim od njegovih tragova.' AS hint
  ...
  UNION ALL SELECT 'Računar' AS left_word, 'Tastatura' AS right_word, 'Asocijacija' AS relation, 'Tehnologija' AS category, 'Lako' AS difficulty, 'Povezi pojam "Računar" sa jednim od njegovih tragova.' AS hint
...
SELECT seed.left_word, seed.right_word, seed.relation, seed.category, seed.difficulty, seed.hint
FROM (
  SELECT 'Hronologija' AS left_word, 'Vrijeme' AS right_word, 'Asocijacija' AS relation, 'Istorija' AS category, 'Tesko' AS difficulty, 'Povezi pojam "Hronologija" sa jednim od njegovih tragova.' AS hint
